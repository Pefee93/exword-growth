...
export default function Services() {
    return (
        <div className="relative w-full min-h-screen bg-[#fbfbf9] text-[#1B0624]">
            {/* Light background Dotted Surface */}
            ...
            <div
                ...
                className={cn(
                    ...
                    'bg-[radial-gradient(ellipse_at_center,rgba(255,69,0,0.05),transparent_60%)]',
                    'blur-[60px]',
                )}
            />
            ...
            <div className="relative z-10 flex flex-col items-center pt-10 px-6">
                ...
                <div className="w-full max-w-[1200px] flex justify-start mb-20">
                    <Link to="/" className="flex items-center gap-2 text-[#1B0624]/50 hover:text-[#FF4500] transition-colors group">
                ...
                <div className="w-full max-w-[1200px] mb-20 md:mb-32">
                    <div className="flex items-center gap-4 mb-8">
                        <span className="font-sans text-[10px] font-semibold tracking-[0.2em] text-[#FF4500] uppercase opacity-90">[ Our Expertise ]</span>
                        <div className="h-[1px] w-16 bg-gradient-to-r from-[#FF4500]/30 to-transparent" />
                    </div>
                    <h1 className="font-['Instrument_Serif'] font-light text-[60px] md:text-[100px] leading-[0.9] tracking-[-0.03em] mb-8 max-w-[900px] text-[#1B0624]">
                        Organic growth for SaaS,<br />from the ground up.
                    </h1>
                    <p className="font-sans text-[16px] md:text-[20px] text-[#1B0624]/60 font-medium leading-[1.6] max-w-[700px]">
                ...
                <div className="w-full max-w-[1200px] flex flex-col gap-8 pb-16">
                    {services.map((service, index) => (
                        <div
                            key={index}
                            className="group flex flex-col md:flex-row gap-8 md:gap-16 p-8 md:p-12 border border-[#1B0624]/5 bg-white/60 backdrop-blur-md shadow-[0_12px_40px_rgba(0,0,0,0.02)] rounded-[2.5rem] hover:bg-white transition-colors relative overflow-hidden cursor-default"
                        >
                            {/* Hover accent line */}
                            <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-[#FF4500] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

                            <div className="md:w-1/3 flex flex-col gap-4">
                                <span className="font-sans text-[12px] text-[#FF4500] font-semibold tracking-widest uppercase mb-2">
                                    [ Service {service.id} ]
                                </span>
                                <h2 className="font-['Instrument_Serif'] text-[36px] md:text-[48px] text-[#1B0624] leading-[1.1] tracking-tight group-hover:pl-2 transition-all duration-300">
                                    {service.title}
                                </h2>
                                <h3 className="font-sans text-[14px] text-[#1B0624]/50 uppercase tracking-widest font-semibold mt-auto mb-4 md:mb-0">
                                    // {service.tagline}
                                </h3>
                            </div>

                            <div className="md:w-2/3 flex flex-col gap-8">
                                <p className="text-[#1B0624]/80 font-sans text-[15px] md:text-[16px] leading-[1.8] font-medium">
                                    {service.description}
                                </p>

                                <div>
                                    <span className="font-sans text-[12px] text-[#1B0624]/40 uppercase tracking-widest font-semibold mb-4 block">Key Deliverables</span>
                                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {service.deliverables.map((item, i) => (
                                            <li key={i} className="flex items-center gap-3">
                                                <div className="w-1.5 h-1.5 rounded-full bg-[#FF4500]/80 group-hover:bg-[#FF4500] transition-colors" />
                                                <span className="font-sans text-[14px] text-[#1B0624]/80 font-medium">{item}</span>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
                ...
                <div className="w-full max-w-[1200px] flex justify-center border-t border-[#1B0624]/10 pt-16 pb-32">
                    <Link to="/" className="group relative overflow-hidden bg-[#1B0624] text-white font-sans font-medium text-[15px] px-[32px] py-[16px] rounded-[1000px] cursor-pointer shadow-[0_10px_30px_rgba(27,6,36,0.1)] hover:shadow-none transition-shadow">
                        <span className="relative z-10 transition-colors duration-300 group-hover:text-white flex items-center gap-2">
                            <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
                            Back to Home
                        </span>
                        <div className="absolute inset-0 bg-[#FF4500] scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] z-0" />
                    </Link>
                </div>
            </div>
        </div>
